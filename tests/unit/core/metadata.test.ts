...
describe('Cell Metadata', () => {
  let engine: FormulaEngine<TestMetadata>;

  beforeEach(() => {
    engine = FormulaEngine.buildEmpty<TestMetadata>();
  ...
  describe('Basic Operations', () => {
    ...
    it('should update existing metadata', () => {
      ...
      expect(engine.getCellMetadata(addr)).toEqual({ comment: 'First comment' });

      engine.setCellMetadata(addr, { comment: 'Updated comment' });
      expect(engine.getCellMetadata(addr)).toEqual({ comment: 'Updated comment' });
    });
    ...
    it('should handle metadata for multiple cells', () => {
      const addr1 = { workbookName: 'wb1', sheetName: 'sheet1', colIndex: 0, rowIndex: 0 };
      const addr2 = { workbookName: 'wb1', sheetName: 'sheet1', colIndex: 1, rowIndex: 0 };
      const addr3 = { workbookName: 'wb1', sheetName: 'sheet1', colIndex: 0, rowIndex: 1 };
      ...
      expect(engine.getCellMetadata(addr3)).toEqual({ link: { url: 'https://example.com' } });
    });
  });

  describe('Paste Operations', () => {
    it('should copy metadata when pasting cells', () => {
      const source = { workbookName: 'wb1', sheetName: 'sheet1', colIndex: 0, rowIndex: 0 };
      const target = { workbookName: 'wb1', sheetName: 'sheet1', colIndex: 1, rowIndex: 0 };
    ...
    it('should copy metadata for multi-cell paste', () => {
      const source1 = { workbookName: 'wb1', sheetName: 'sheet1', colIndex: 0, rowIndex: 0 };
      const source2 = { workbookName: 'wb1', sheetName: 'sheet1', colIndex: 1, rowIndex: 0 };
      ...
      const targetStart = { workbookName: 'wb1', sheetName: 'sheet1', colIndex: 0, rowIndex: 1 };
      engine.pasteCells([source1, source2], targetStart, { include: 'all' });

      const target1 = { workbookName: 'wb1', sheetName: 'sheet1', colIndex: 0, rowIndex: 1 };
      const target2 = { workbookName: 'wb1', sheetName: 'sheet1', colIndex: 1, rowIndex: 1 };
  ...
  describe('AutoFill Operations', () => {
    it('should copy metadata when autofilling', () => {
      ...
      engine.autoFill(
        ...
        { start: { col: 0, row: 0 }, end: { col: { type: 'number', value: 0 }, row: { type: 'number', value: 0 } } },
        [
          { start: { col: 0, row: 1 }, end: { col: { type: 'number', value: 0 }, row: { type: 'number', value: 2 } } },
        ],
        'down'
      );

      const target1 = { workbookName: 'wb1', sheetName: 'sheet1', colIndex: 0, rowIndex: 1 };
      const target2 = { workbookName: 'wb1', sheetName: 'sheet1', colIndex: 0, rowIndex: 2 };

      expect(engine.getCellMetadata(target1)).toEqual({ comment: 'Source cell' });
      expect(engine.getCellMetadata(target2)).toEqual({ comment: 'Source cell' });
    });
  });
  ...
  describe('Serialization', () => {
    it('should serialize and deserialize metadata', () => {
      ...
      const newEngine = FormulaEngine.buildEmpty<TestMetadata>();
  ...
  describe('Workbook Cloning', () => {
    it('should clone metadata when cloning workbook', () => {
      ...
      const clonedAddr = { workbookName: 'wb2', sheetName: 'sheet1', colIndex: 0, rowIndex: 0 };
    ...
    it('should clone all metadata in workbook', () => {
      ...
      const cloned1 = { workbookName: 'wb2', sheetName: 'sheet1', colIndex: 0, rowIndex: 0 };
      const cloned2 = { workbookName: 'wb2', sheetName: 'sheet1', colIndex: 1, rowIndex: 0 };
    ...
    it('should keep original and cloned metadata independent', () => {
      const originalAddr = { workbookName: 'wb1', sheetName: 'sheet1', colIndex: 0, rowIndex: 0 };
      ...
      expect(engine.getCellMetadata(clonedAddr)).toEqual({ comment: 'Original' });
    });
  });
  ...
  describe('Multi-Sheet Operations', () => {
    ...
    it('should handle metadata across multiple sheets', () => {
      const addr1 = { workbookName: 'wb1', sheetName: 'sheet1', colIndex: 0, rowIndex: 0 };
      const addr2 = { workbookName: 'wb1', sheetName: 'sheet2', colIndex: 0, rowIndex: 0 };
    ...
    it('should copy metadata between sheets', () => {
      const source = { workbookName: 'wb1', sheetName: 'sheet1', colIndex: 0, rowIndex: 0 };
      const target = { workbookName: 'wb1', sheetName: 'sheet2', colIndex: 0, rowIndex: 0 };
      ...
      expect(engine.getCellMetadata(target)).toEqual({ link: { url: 'https://example.com' } });
    });
  });
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle metadata for cells with formulas', () => {
      ...
      expect(engine.getCellMetadata(addr)).toEqual({ comment: 'This is a formula' });
    });
  });
  ...
  describe('Sheet Metadata', () => {
    beforeEach(() => {
      engine = FormulaEngine.buildEmpty<TestMetadata, { textBoxes?: string[]; frozen?: boolean }, unknown>();
      engine.addWorkbook('wb1');
      engine.addSheet({ workbookName: 'wb1', sheetName: 'sheet1' });
    });
    ...
    it('should set and get sheet metadata', () => {
      ...
      engine.setSheetMetadata({ workbookName: 'wb1', sheetName: 'sheet1' }, sheetMeta);
      const retrieved = engine.getSheetMetadata({ workbookName: 'wb1', sheetName: 'sheet1' });
    ...
    it('should preserve sheet metadata when renaming sheet', () => {
      engine.setSheetMetadata({ workbookName: 'wb1', sheetName: 'sheet1' }, { textBoxes: ['box1'] });
      
      engine.renameSheet({ workbookName: 'wb1', sheetName: 'sheet1', newSheetName: 'renamed' });
      
      const retrieved = engine.getSheetMetadata({ workbookName: 'wb1', sheetName: 'renamed' });
      expect(retrieved).toEqual({ textBoxes: ['box1'] });
    });

    it('should clone sheet metadata when cloning workbook', () => {
      engine.setSheetMetadata({ workbookName: 'wb1', sheetName: 'sheet1' }, { 
        textBoxes: ['box1', 'box2'],
        frozen: true 
      });
      ...
      const clonedMeta = engine.getSheetMetadata({ workbookName: 'wb2', sheetName: 'sheet1' });
    ...
    it('should serialize and deserialize sheet metadata', () => {
      ...
      const serialized = engine.serializeEngine();
      const newEngine = FormulaEngine.buildEmpty<TestMetadata, { textBoxes?: string[]; frozen?: boolean }, unknown>();
      newEngine.resetToSerializedEngine(serialized);

      const retrieved = newEngine.getSheetMetadata({ workbookName: 'wb1', sheetName: 'sheet1' });
    ...
    it('should handle multiple sheets with different metadata', () => {
      ...
      engine.setSheetMetadata({ workbookName: 'wb1', sheetName: 'sheet1' }, { textBoxes: ['box1'] });
      engine.setSheetMetadata({ workbookName: 'wb1', sheetName: 'sheet2' }, { frozen: true });

      expect(engine.getSheetMetadata({ workbookName: 'wb1', sheetName: 'sheet1' })).toEqual({ textBoxes: ['box1'] });
      expect(engine.getSheetMetadata({ workbookName: 'wb1', sheetName: 'sheet2' })).toEqual({ frozen: true });
    });
  });

  describe('Workbook Metadata', () => {
    beforeEach(() => {
      engine = FormulaEngine.buildEmpty<TestMetadata, unknown, { theme?: string; author?: string }>();
      engine.addWorkbook('wb1');
      engine.addSheet({ workbookName: 'wb1', sheetName: 'sheet1' });
    });
    ...
    it('should preserve workbook metadata when renaming workbook', () => {
      ...
      engine.renameWorkbook({ workbookName: 'wb1', newWorkbookName: 'renamed' });
    ...
    it('should serialize and deserialize workbook metadata', () => {
      ...
      const serialized = engine.serializeEngine();
      const newEngine = FormulaEngine.buildEmpty<TestMetadata, unknown, { theme?: string; author?: string }>();
      newEngine.resetToSerializedEngine(serialized);
  ...
  describe('Combined Metadata', () => {
    ...
    it('should handle cell, sheet, and workbook metadata together', () => {
      ...
      combinedEngine.setSheetMetadata({ workbookName: 'wb1', sheetName: 'sheet1' }, { textBoxes: ['box1'] });
      combinedEngine.setWorkbookMetadata('wb1', { theme: 'dark' });

      expect(combinedEngine.getCellMetadata(addr)).toEqual({ comment: 'formatted' });
      expect(combinedEngine.getSheetMetadata({ workbookName: 'wb1', sheetName: 'sheet1' })).toEqual({ textBoxes: ['box1'] });
      expect(combinedEngine.getWorkbookMetadata('wb1')).toEqual({ theme: 'dark' });
    });
    ...
    it('should clone all three metadata types correctly', () => {
      ...
      combinedEngine.setCellContent(addr, 'Test');
      combinedEngine.setCellMetadata(addr, { comment: 'formatted' });
      combinedEngine.setSheetMetadata({ workbookName: 'wb1', sheetName: 'sheet1' }, { textBoxes: ['box1', 'box2'] });
      combinedEngine.setWorkbookMetadata('wb1', { theme: 'light' });

      combinedEngine.cloneWorkbook('wb1', 'wb2');

      const clonedAddr = { workbookName: 'wb2', sheetName: 'sheet1', colIndex: 0, rowIndex: 0 };
      expect(combinedEngine.getCellMetadata(clonedAddr)).toEqual({ comment: 'formatted' });
      expect(combinedEngine.getSheetMetadata({ workbookName: 'wb2', sheetName: 'sheet1' })).toEqual({ textBoxes: ['box1', 'box2'] });
      expect(combinedEngine.getWorkbookMetadata('wb2')).toEqual({ theme: 'light' });
    });
    ...
    it('should serialize all three metadata types', () => {
      ...
      combinedEngine.setSheetMetadata({ workbookName: 'wb1', sheetName: 'sheet1' }, { textBoxes: ['tb1'] });
      ...
      const restoredAddr = { workbookName: 'wb1', sheetName: 'sheet1', colIndex: 0, rowIndex: 0 };
      expect(newEngine.getCellValue(restoredAddr)).toBe('Data');
      expect(newEngine.getCellMetadata(restoredAddr)).toEqual({ comment: 'bold' });
      expect(newEngine.getSheetMetadata({ workbookName: 'wb1', sheetName: 'sheet1' })).toEqual({ textBoxes: ['tb1'] });
    ...
    it('should keep cloned metadata independent from original', () => {
      ...
      // Modify original
      combinedEngine.setSheetMetadata({ workbookName: 'wb1', sheetName: 'sheet1' }, { textBoxes: ['modified'] });
      combinedEngine.setWorkbookMetadata('wb1', { theme: 'changed' });

      // Cloned should remain unchanged
      expect(combinedEngine.getSheetMetadata({ workbookName: 'wb2', sheetName: 'sheet1' })).toEqual({ textBoxes: ['box1'] });
      expect(combinedEngine.getWorkbookMetadata('wb2')).toEqual({ theme: 'original' });
    });
  });
});
