...
describe("Tables", () => {
  ...
  test("should update structured references when a table header is renamed", () => {
    engine.setSheetContent(
      { workbookName, sheetName },
      new Map<string, SerializedCellValue>([
        ["A1", "Product"],
    ...
    expect(content.get("E1")).toBe("=SUM(Products[Cost])");
    expect(content.get("C2")).toBe("=[@Cost]*2");
    expect(content.get("C3")).toBe("=[@Cost]*2");
    expect(cell("E1")).toBe(25);
    expect(cell("C2")).toBe(20);
    expect(cell("C3")).toBe(30);
  });

  test("should synchronize table headers during a full sheet update", () => {
    engine.setSheetContent(
      { workbookName, sheetName },
      new Map<string, SerializedCellValue>([
        ["A1", "Product"],
    ...
    updatedContent.set("B1", "Cost");
    engine.setSheetContent({ workbookName, sheetName }, updatedContent);

    expect(
      Array.from(
        engine.getTable({ tableName: "Products", workbookName })!.headers.keys()
      )
    ).toEqual(["Product", "Cost"]);
    expect(
      engine.getSheetSerialized({ workbookName, sheetName }).get("D1")
    ).toBe("=SUM(Products[Cost])");
    expect(cell("D1")).toBe(10);
  });

  test("should populate unique default names for blank table headers", () => {
    engine.setSheetContent(
      { workbookName, sheetName },
      new Map<string, SerializedCellValue>([
        ["A2", "Widget"],
    ...
    const table = engine.getTable({ tableName: "Products", workbookName })!;
    expect(Array.from(table.headers.keys())).toEqual(["Column 1", "Column 2"]);
    expect(cell("A1")).toBe("Column 1");
    expect(cell("B1")).toBe("Column 2");

    setCellContent("D1", "=SUM(Products[Column 2])");
    expect(cell("D1")).toBe(10);
  });

  test("should reject duplicate table headers when creating a table", () => {
    engine.setSheetContent(
      { workbookName, sheetName },
      new Map<string, SerializedCellValue>([
        ["A1", "Value"],
    ...
    ).toThrow('Duplicate table header "Value"');
    expect(engine.hasTable({ tableName: "Data", workbookName })).toBe(false);
  });

  test("should reject edits that would create duplicate table headers", () => {
    engine.setSheetContent(
      { workbookName, sheetName },
      new Map<string, SerializedCellValue>([
        ["A1", "Value"],
    ...
    expect(() => setCellContent("B1", "Value")).toThrow(
      'Duplicate table header "Value"'
    );
    expect(cell("B1")).toBe("Amount");
    expect(Array.from(engine.getTable({ tableName: "Data", workbookName })!.headers.keys())).toEqual([
      "Value",
      "Amount",
    ]);
  });
