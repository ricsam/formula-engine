...
describe("Evaluator", () => {
  ...
  describe("Value nodes", () => {
    ...
    test("should evaluate error node", () => {
      const node = createTestErrorNode(FormulaError.DIV0);

      const result = evaluator.evaluate(node, context);
      expect(result.value).toBe(FormulaError.DIV0);
    });
  });
  ...
  describe("Range nodes", () => {
    test("should evaluate range", () => {
      ...
      expect(result.type).toBe("2d-array");
      expect((result as any).dimensions).toEqual({ rows: 1, cols: 3 });
  ...
  describe("Function nodes", () => {
    ...
    test("should handle unknown function", () => {
      ...
      const result = evaluator.evaluate(node, context);
      expect(result.value).toBe(FormulaError.NAME);
    });
    ...
    test("should validate argument count", () => {
      ...
      const result = evaluator.evaluate(node, context);
      expect(result.value).toBe(FormulaError.VALUE);
    });
  ...
  describe("Array nodes", () => {
    test("should evaluate array literal", () => {
      ...
      const result = evaluator.evaluate(node, context);
      expect(result.value as unknown as number[][]).toEqual([
        [1, 2],
        [3, 4],
      ]);
      expect(result.type).toBe("2d-array");
      expect((result as any).dimensions).toEqual({ rows: 2, cols: 2 });
    });
  });
  ...
  describe("Type coercion", () => {
    test("should coerce values to numbers", () => {
      ...
      expect(evaluator.coerceToNumber("abc")).toBe(FormulaError.VALUE);
    });
  ...
  describe("Error propagation", () => {
    test("should propagate errors through operations", () => {
      const node = createTestBinaryOpNode(
        "+",
        createTestErrorNode(FormulaError.VALUE),
        createTestValueNode(10, "number")
      );
    ...
    test("should propagate errors through functions", () => {
      const node = createTestFunctionNode("SUM", [
        createTestValueNode(10, "number"),
        createTestErrorNode(FormulaError.REF),
        createTestValueNode(20, "number"),
      ]);

      const result = evaluator.evaluate(node, context);
      expect(result.value).toBe(FormulaError.REF);
    });
  });
