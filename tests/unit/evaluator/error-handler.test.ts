...
describe("Error Handler", () => {
  ...
  describe("mapJSErrorToFormulaError", () => {
    test("should map division by zero errors", () => {
      expect(mapJSErrorToFormulaError(new Error("division by zero"))).toBe(
        FormulaError.DIV0
      );
      expect(mapJSErrorToFormulaError(new Error("Cannot divide by zero"))).toBe(
        FormulaError.DIV0
      );
    });

    test("should map reference errors", () => {
      expect(mapJSErrorToFormulaError(new Error("Invalid reference"))).toBe(
        FormulaError.REF
      );
      expect(
        mapJSErrorToFormulaError(new Error("Cell reference not found"))
      ).toBe(FormulaError.REF);
    });

    test("should map name errors", () => {
      expect(mapJSErrorToFormulaError(new Error("Invalid name"))).toBe(
        FormulaError.NAME
      );
      expect(mapJSErrorToFormulaError(new Error("Unknown function: FOO"))).toBe(
        FormulaError.NAME
      );
    });

    test("should map numeric errors", () => {
      expect(mapJSErrorToFormulaError(new Error("Invalid number"))).toBe(
        FormulaError.NUM
      );
      expect(mapJSErrorToFormulaError(new Error("Result is NaN"))).toBe(
        FormulaError.NUM
      );
      expect(mapJSErrorToFormulaError(new Error("Infinity"))).toBe(
        FormulaError.NUM
      );
    });

    test("should map type errors", () => {
      expect(mapJSErrorToFormulaError(new Error("Invalid argument type"))).toBe(
        FormulaError.VALUE
      );
      expect(mapJSErrorToFormulaError(new Error("Type mismatch"))).toBe(
        FormulaError.VALUE
      );
    });
    ...
    test("should map circular reference errors", () => {
      expect(
        mapJSErrorToFormulaError(new Error("Circular reference detected"))
      ).toBe(FormulaError.CYCLE);
      expect(mapJSErrorToFormulaError(new Error("Formula has cycle"))).toBe(
        FormulaError.CYCLE
      );
    });

    test("should map N/A errors", () => {
      expect(mapJSErrorToFormulaError(new Error("Value not available"))).toBe(
        FormulaError.NA
      );
      expect(mapJSErrorToFormulaError(new Error("N/A"))).toBe(FormulaError.NA);
    });

    test("should default to general error", () => {
      expect(mapJSErrorToFormulaError(new Error("Something went wrong"))).toBe(
        FormulaError.ERROR
      );
      expect(mapJSErrorToFormulaError(new Error(""))).toBe(FormulaError.ERROR);
    });
  });

  describe("createFormulaError", () => {
    test("should create error with context", () => {
      const error = createFormulaError(FormulaError.VALUE, {
        message: "Type mismatch",
        functionName: "SUM",
        argumentIndex: 1,
      });

      expect(error.type).toBe(FormulaError.VALUE);
    ...
    test("should create error without context", () => {
      const error = createFormulaError(FormulaError.DIV0);
      expect(error.type).toBe(FormulaError.DIV0);
  ...
  describe("propagateError", () => {
    test("should return first error found", () => {
      ...
      expect(propagateError(values)).toBe(FormulaError.VALUE);
    });
  ...
  describe("propagateError2D", () => {
    test("should return first error found in 2D array", () => {
      ...
      expect(propagateError2D(values)).toBe(FormulaError.NUM);
    });
  ...
  describe("validateNumericArgument", () => {
    ...
    test("should return error for non-finite numbers", () => {
      expect(validateNumericArgument(Infinity)).toBe(FormulaError.NUM);
      expect(validateNumericArgument(-Infinity)).toBe(FormulaError.NUM);
      expect(validateNumericArgument(NaN)).toBe(FormulaError.NUM);
    });

    test("should return error for non-numeric strings", () => {
      expect(validateNumericArgument("abc")).toBe(FormulaError.VALUE);
      expect(validateNumericArgument("12.34.56")).toBe(FormulaError.VALUE);
    });

    test("should propagate errors", () => {
      expect(validateNumericArgument("#DIV/0!")).toBe(FormulaError.DIV0);
      expect(validateNumericArgument("#REF!")).toBe(FormulaError.REF);
    });
  });
  ...
  describe("validateTextArgument", () => {
    ...
    test("should propagate errors", () => {
      expect(validateTextArgument("#VALUE!")).toBe(FormulaError.VALUE);
    });
  });
  ...
  describe("validateBooleanArgument", () => {
    ...
    test("should propagate errors", () => {
      expect(validateBooleanArgument(FormulaError.NAME)).toBe(
        FormulaError.NAME
      );
    });
  });
  ...
  describe("formatError", () => {
    test("should format simple error", () => {
      expect(formatError(FormulaError.DIV0)).toBe("#DIV/0!");
    });
  ...
  describe("ErrorHandler", () => {
    test("should record and retrieve errors", () => {
      ...
      handler.recordError("A1", FormulaError.DIV0, { formula: "=1/0" });
    ...
    test("should clear errors", () => {
      ...
      handler.recordError("A1", FormulaError.VALUE);
      handler.recordError("B1", FormulaError.REF);
    ...
    test("should get all error cells", () => {
      ...
      handler.recordError("A1", FormulaError.VALUE);
      handler.recordError("B2", FormulaError.REF);
      handler.recordError("C3", FormulaError.NAME);
    ...
    test("should handle errors with default strategy", () => {
      ...
      expect(handler.handleError(FormulaError.DIV0)).toBe(FormulaError.DIV0);

      const extendedError = createFormulaError(FormulaError.VALUE);
      expect(handler.handleError(extendedError)).toBe(FormulaError.VALUE);
    });

    test("should handle errors with suppress strategy", () => {
      const handler = new ErrorHandler(suppressErrorsRecovery);

      expect(handler.handleError(FormulaError.DIV0)).toBe(0);
      expect(handler.handleError(FormulaError.VALUE)).toBe(0);
      expect(handler.handleError(FormulaError.NA)).toBeUndefined();
    });

    test("should change error recovery strategy", () => {
      const handler = new ErrorHandler();

      expect(handler.handleError(FormulaError.VALUE)).toBe(FormulaError.VALUE);
