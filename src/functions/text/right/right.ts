...
import { substringOperation, createTextSpilledResult } from "../text-helpers";
  ...
  evaluate: function (node, context): FunctionEvaluationResult {
    ...
    if (textResult.type === "spilled-values" || numCharsResult.type === "spilled-values") {
      return createTextSpilledResult.call(this, {
        operation: (text, numChars) => substringOperation(text, numChars, "right"),
        textResult,
        numCharsResult,
        context,
        functionName: "RIGHT",
    ...
    try {
      return {
        type: "value",
        result: substringOperation(textResult, numCharsResult, "right"),
      };
    } catch (error) {
      return {
        type: "error",
