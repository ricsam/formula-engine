...
export const FIND: FunctionDefinition = {
  ...
  evaluate: (args: CellValue[]): CellValue => {
    ...
    if (findText === undefined || findText === null || 
        withinText === undefined || withinText === null) {
      return '#VALUE!';
    }
...
export const SUBSTITUTE: FunctionDefinition = {
  ...
  evaluate: (args: CellValue[]): CellValue => {
    ...
    if (text === undefined || text === null || 
        oldText === undefined || oldText === null ||
        newText === undefined || newText === null) {
      return String(text || '');
    }
    
    if (instanceNum !== undefined && (typeof instanceNum !== 'number' || instanceNum < 1)) {
      return '#VALUE!';
    }
...
export const REPLACE: FunctionDefinition = {
  ...
  evaluate: (args: CellValue[]): CellValue => {
    ...
    return oldStr.substring(0, start) + newStr + oldStr.substring(start + length);
  }
};
