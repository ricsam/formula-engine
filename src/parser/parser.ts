...
export class Parser {
  ...
  private parseTableReference(tableName: string, startPos: number): ASTNode {
    ...
    let selector: "#All" | "#Data" | "#Headers" | "#ThisRow" | undefined;
    let columnName: string | undefined;
    let isCurrentRow = false;
    
    // Check for complex syntax like [[#Headers],[Column1]]
    if (this.tokens.match("LBRACKET")) {
      ...
      if (!this.tokens.match("IDENTIFIER")) {
        throw new ParseError("Expected column name after @", this.tokens.peek().position);
      }
      columnName = this.tokens.consume().value;
      ...
      // Simple column reference like Table1[Column1]
      columnName = this.tokens.consume().value;
  ...
  /**
   * Parse current row reference (e.g., [@Column] or just @Column)
   */
  private parseCurrentRowReference(): ASTNode {
    ...
    if (this.tokens.match("LBRACKET")) {
      ...
      const columnName = this.tokens.consume().value;
  ...
  private parseTableSelector(): ASTNode {
    ...
    if (!["#All", "#Data", "#Headers", "#ThisRow"].includes(selector.toUpperCase())) {
