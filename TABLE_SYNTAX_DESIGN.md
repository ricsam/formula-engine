...
export class FormulaEngine {
  ...
  changeTable(
    tableName: string,
    sheetName: string,
    newRange: SpreadsheetRange,
  ): void;

  removeTable(tableName: string, sheetName: string): void;
}
```
...
export class TableReferenceResolver {
  ...
  ): SpreadsheetRange | SimpleCellAddress;
    ...
    if (node.isCurrentRow) {
      if (!context.currentTable || context.currentTableRow === undefined) {
        throw new FormulaError(
          "#REF!",
          "Current row reference outside table context"
        );
      }
...
export class DependencyGraph {
  ...
  addTable(name: string, sheetName: string): string;

  // Creates a unique key for a table (following named expression pattern)
  static getTableKey(name: string, sheetName: string): string;
...
describe("Table Management", () => {
  test("should handle table CRUD operations", () => {
    ...
    const sheetName = engine.getSheetId(engine.addSheet("Test"));

    // Set up header data
    engine.setCellContent({ sheet: sheetName, col: 0, row: 0 }, "Name");
    engine.setCellContent({ sheet: sheetName, col: 1, row: 0 }, "Sales");

    // Test table creation
    const range = {
      start: { sheet: sheetName, col: 0, row: 0 },
      end: { sheet: sheetName, col: 1, row: 5 },
    };

    expect(engine.isItPossibleToAddTable("SalesData", range, sheetName)).toBe(
      true
    );
    engine.addTable("SalesData", range, sheetName);

    // Test table retrieval
    const table = engine.getTable("SalesData", sheetName);
    expect(table?.name).toBe("SalesData");
    expect(table?.range).toEqual(range);

    // Test table listing
    const tables = engine.listTables(sheetName);
    expect(tables).toContain("SalesData");

    // Test table modification
    const newRange = {
      start: { sheet: sheetName, col: 0, row: 0 },
      end: { sheet: sheetName, col: 2, row: 8 },
    };
    engine.changeTable("SalesData", newRange, sheetName);
    expect(engine.getTable("SalesData", sheetName)?.range).toEqual(newRange);

    // Test table removal
    engine.removeTable("SalesData", sheetName);
    expect(engine.listTables(sheetName)).not.toContain("SalesData");
  });
  ...
  test("should handle table validation and conflicts", () => {
    ...
    const changes = engine.addTable("SalesData", range, sheetName, {
      overwrite: true,
    });
  ...
  test("should get table column names from headers", () => {
    ...
    engine.setCellContent({ sheet: sheetName, col: 0, row: 0 }, "Product");
    engine.setCellContent({ sheet: sheetName, col: 1, row: 0 }, "Price");
    engine.setCellContent({ sheet: sheetName, col: 2, row: 0 }, "Quantity");

    const range = {
      start: { sheet: sheetName, col: 0, row: 0 },
      end: { sheet: sheetName, col: 2, row: 5 },
    };
    engine.addTable("Inventory", range, sheetName);

    const columnNames = engine.getTableColumnNames("Inventory", sheetName);
...
describe("Table operations", () => {
  ...
  test("should create correct table keys", () => {
    expect(DependencyGraph.getTableKey("SalesData", 0)).toBe(
      "table:0:SalesData"
    );
    expect(DependencyGraph.getTableKey("Inventory", 1)).toBe(
      "table:1:Inventory"
    );
  });
...
describe("Structured Reference Parsing", () => {
  ...
  test("should extract table references from formulas", () => {
    const tables = extractTableReferences(
      "Table1[Sales]*Table1[Quantity]+Table2[Discount]"
    );
...
describe("Structured references", () => {
  ...
  test("should handle missing column reference", () => {
    ...
    const node = createTestStructuredReferenceNode(
      "SalesData",
      "UnknownColumn"
    );
    const result = evaluator.evaluate(node, context);
...
describe("Table dependency tracking", () => {
  test("should track dependencies through table references", () => {
    ...
    // Set up table data
    engine.setCellContent({ sheet: sheetName, col: 0, row: 0 }, "Amount");
    engine.setCellContent({ sheet: sheetName, col: 0, row: 1 }, 100);
    engine.setCellContent({ sheet: sheetName, col: 0, row: 2 }, 200);

    // Create table
    const range = {
      start: { sheet: sheetName, col: 0, row: 0 },
      end: { sheet: sheetName, col: 0, row: 2 },
    };
    engine.addTable("SalesData", range, sheetName);

    // Create formula referencing table
    engine.setCellContent(
      { sheet: sheetName, col: 1, row: 0 },
      "=SUM(SalesData[Amount])"
    );

    // Test dependency tracking
    const precedents = engine.getCellPrecedents({
      sheet: sheetName,
      col: 1,
      row: 0,
    });
  ...
  test("should update dependencies when table is resized", () => {
    ...
    const newRange = {
      start: { sheet: sheetName, col: 0, row: 0 },
      end: { sheet: sheetName, col: 0, row: 5 },
    };
    engine.changeTable("SalesData", newRange, sheetName);

    // Verify dependencies are updated
    const dependents = engine.getCellDependents({
      sheet: sheetName,
      col: 0,
      row: 4,
    }); // New cell in table
...
describe("Table Operations Integration", () => {
  test("should handle complete table workflow", () => {
    ...
    // Set up initial data
    engine.setCellContent({ sheet: sheetName, col: 0, row: 0 }, [
    ...
    const range = {
      start: { sheet: sheetName, col: 0, row: 0 },
      end: { sheet: sheetName, col: 2, row: 3 },
    };
    engine.addTable("Inventory", range, sheetName);

    // Test structured reference formulas
    engine.setCellContent(
      { sheet: sheetName, col: 3, row: 1 },
      "=[@Price]*[@Quantity]"
    ); // Current row
    engine.setCellContent(
      { sheet: sheetName, col: 3, row: 4 },
      "=SUM(Inventory[Price])"
    ); // Column sum

    // Verify calculations
    expect(engine.getCellValue({ sheet: sheetName, col: 3, row: 1 })).toBe(50); // 10*5
    expect(engine.getCellValue({ sheet: sheetName, col: 3, row: 4 })).toBe(45); // 10+15+20
    ...
    engine.changeTable("Inventory", newRange, sheetName);

    // Add new data
    engine.setCellContent({ sheet: sheetName, col: 0, row: 4 }, "Widget D");
    engine.setCellContent({ sheet: sheetName, col: 1, row: 4 }, 25);

    // Verify sum updates automatically
    expect(engine.getCellValue({ sheet: sheetName, col: 3, row: 4 })).toBe(70); // 10+15+20+25
  });
  ...
  test("should support all table selectors", () => {
    ...
    const range = {
      start: { sheet: sheetName, col: 0, row: 0 },
      end: { sheet: sheetName, col: 1, row: 3 },
    };
    engine.addTable("Sales", range, sheetName);

    // Test different selectors
    engine.setCellContent(
      { sheet: sheetName, col: 2, row: 0 },
      "=Sales[[#Headers],[Product]]"
    ); // Header
    engine.setCellContent(
      { sheet: sheetName, col: 2, row: 1 },
      "=SUM(Sales[[#Data],[Sales]])"
    ); // Data only
    engine.setCellContent(
      { sheet: sheetName, col: 2, row: 2 },
      "=COUNTA(Sales[[#All],[Product]])"
    ); // All rows

    expect(engine.getCellValue({ sheet: sheetName, col: 2, row: 0 })).toBe(
      "Product"
    );
    expect(engine.getCellValue({ sheet: sheetName, col: 2, row: 1 })).toBe(600); // 100+200+300
    expect(engine.getCellValue({ sheet: sheetName, col: 2, row: 2 })).toBe(4); // Header + 3 data
  });
});
```
...
describe("Excel Table Compatibility", () => {
  test("should match Excel structured reference behavior", () => {
    ...
    excelTestCases.forEach((testCase) => {
      const result = engine.calculateFormula(testCase.formula, sheetName);
...
describe("Table Performance", () => {
  test("should handle large tables efficiently", () => {
    ...
    const largeRange = {
      start: { sheet: sheetName, col: 0, row: 0 },
      end: { sheet: sheetName, col: 9, row: 999 },
    };
    engine.addTable("LargeTable", largeRange, sheetName);

    // Add formulas referencing the table
    engine.setCellContent(
      { sheet: sheetName, col: 10, row: 0 },
      "=SUM(LargeTable[Column1])"
    );
...
describe("Table Error Handling", () => {
  test("should return appropriate error types", () => {
    ...
    expect(() => engine.addTable("", range, sheetName)).toThrow(); // Empty name
    expect(() => engine.addTable("Table1", invalidRange, sheetName)).toThrow(); // Invalid range

    // Test formula errors
    const result1 = engine.calculateFormula(
      "=UnknownTable[Column1]",
      sheetName
    );
    expect(result1).toBe("#TABLE!");

    const result2 = engine.calculateFormula(
      "=Table1[UnknownColumn]",
      sheetName
    );
...
// Tables should have:
isItPossibleToAddTable(name: string, range: SimpleCellRange, sheetName: string): boolean
addTable(name: string, range: SimpleCellRange, sheetName: string): ExportedChange[]
isItPossibleToChangeTable(name: string, newRange: SimpleCellRange, sheetName: string): boolean
changeTable(name: string, newRange: SimpleCellRange, sheetName: string): ExportedChange[]
isItPossibleToRemoveTable(name: string, sheetName: string): boolean
removeTable(name: string, sheetName: string): ExportedChange[]
```
...
const tableKey = DependencyGraph.getTableKey(tableName, sheetName);
this.dependencyGraph.addTable(tableName, sheetName);
```
